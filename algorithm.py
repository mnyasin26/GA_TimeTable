...
file_kelas = 'classes/jadwal_kelas.json'
file_guru = 'classes/jadwal_guru.json'
...
def evolutionary_algorithm():
    ...
    # mengambil data dari file input
    ...
    print('Run', 'cost', cost_function(chromosome), 'chromosome', chromosome)

    dt.write_data(chromosome[0], output_file)
    dt.write_data(chromosome[1], file_guru)
    dt.write_data(chromosome[2], file_kelas)
    ...
    for grupa in chromosome[2]:
        for i in range(len(chromosome[2][grupa])):
            if chromosome[2][grupa][i] > 1:
                group_hard = False
    ...
    print('Average group cost is:', total_group_cost / len(chromosome[2]))
